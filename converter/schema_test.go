...
func TestDefineSchemaMsg(t *testing.T) {
	tt := []struct {
	...
	}{
		{
			...
			"message test {required binary VARIANTKEY (STRING); required binary CHROM (STRING); required int32 POS; required binary REF (STRING); required binary ALT (STRING); required int32 QUAL; required binary FILTER (STRING); required binary SAMPLE (STRING); required int32 NUMALTS; required boolean IS_PHASED; required binary PHASE_ID (STRING); required boolean PASS; required boolean IS_SV; required binary SVTYPE (STRING); required int32 END; required binary AS_RAW_MQ (STRING); required boolean DS; required double AF; required int32 AC;}",
		},
		{
			map[string]*vcfio.InfoVcf{},
			"message test {required binary VARIANTKEY (STRING); required binary CHROM (STRING); required int32 POS; required binary REF (STRING); required binary ALT (STRING); required int32 QUAL; required binary FILTER (STRING); required binary SAMPLE (STRING); required int32 NUMALTS; required boolean IS_PHASED; required binary PHASE_ID (STRING); required boolean PASS; required boolean IS_SV; required binary SVTYPE (STRING); required int32 END;}",
		},
	}
	for i, test := range tt {
		header := vcfio.Header{
